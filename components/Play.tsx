...
const Play: React.FC<PlayProps> = ({ user, onUpdateUser, playSound }) => {
  ...
  const fetchQuestionBatch = useCallback(async (subject: Subject) => {
    ...
    const promises = Array(QUESTIONS_TO_FETCH).fill(0).map(() => generateMCQ(topic));
    let newQuestions = (await Promise.all(promises)).filter((q): q is Question => q !== null);
    
    if (subject === 'English' && Math.random() < 0.25) { // 25% chance for a funny question in the batch
      const funnyQ = await generateMCQ("a funny, absurd, or silly pop-culture topic");
      if (funnyQ) {
        const randomIndex = Math.floor(Math.random() * (newQuestions.length + 1));
        newQuestions.splice(randomIndex, 0, funnyQ);
      }
    }
  ...
  const determineAndAwardGift = () => {
    ...
    onUpdateUser(user.id, (currentUserState) => {
        let updatedUser = {...currentUserState};
        switch (chosenGift.type) {
            case 'DOUBLE_CREDS': updatedUser.creds = Math.floor(updatedUser.creds * 2); break;
            case '1000_CREDS': updatedUser.creds += 1000; break;
            case '10_CREDS': updatedUser.creds += 10; break;
            case 'SHIELD_ITEM':
            case 'XP_BOOSTER_ITEM':
               const item = chosenGift.item;
               if (item) {
    ...
    setSurpriseGift({name: chosenGift.name, icon: chosenGift.item?.icon || <GiftIcon className="w-8 h-8"/> });
    setShowSurpriseBox(true);
};


  const loadNextQuestionFromQueue = useCallback(() => {
    // A bonus question can only appear if the user has a streak of 2 or more.
  ...
  const handleSubjectSelect = async (subject: Subject) => {
    ...
    if (subject === 'English') {
      ...
      loadNextQuestionFromQueue(); // This will pull the first question
      setIsLoading(false);
    }
  };
  ...
  const handleAnswer = (answer: string) => {
    ...
    onUpdateUser(user.id, (currentUserState) => {
        let xpGained = 0;
        let credsGained = 0;
        let newOverallStreak = currentUserState.streak;
        ...
        if (isCorrect) {
            ...
            credsGained = -Math.min(currentUserState.creds, credsLost);
            newOverallStreak = 0;
        }

        const newXp = currentUserState.xp + xpGained;
        const xpForNextLevel = getXpForNextLevel(currentUserState.level);
        let newLevel = currentUserState.level;
        ...
        return {
            ...
            xp: newXp,
            level: newLevel,
            creds: currentUserState.creds + credsGained,
      ...
      {isLoading ? (
        ...
        question && (
          <div className="animate-fade-in flex-1">
            <div className={`mb-6 p-4 border rounded-lg bg-black/30 transition-all duration-500 ${isBonusQuestion ? 'border-yellow-400 shadow-lg shadow-yellow-500/30' : 'border-cyan-500/50'}`}>
              <div className="flex justify-between items-center">
                <p className="text-sm text-pink-400 mb-2 font-orbitron">// Topic: {question.topic}</p>
                {isBonusQuestion && <p className="text-sm font-bold text-yellow-300 bg-yellow-900/50 px-2 py-1 rounded animate-pulse">BONUS ROUND</p>}
              </div>
